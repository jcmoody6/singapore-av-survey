...
Data <- Data %>% mutate(DOB = as.numeric(as.character(age)),
                        ...
                        HHInc = ifelse(hhinc==19, NA, hhinc),
                        HHIncLow = ifelse(hhinc == 1 | hhinc == 2, 1, 0),
                        HHIncMid = ifelse(hhinc == 3 | hhinc == 4 | hhinc == 5, 1, 0),
                        Inc1 = ifelse(hhinc %in% 1:2, 1, 0), # <S$2500
                        Inc2 = ifelse(hhinc %in% 3:6, 1, 0), #S$2500-7000
                        Inc3 = ifelse(hhinc %in% 7:10, 1, 0), #S$7000-11000
                        Inc4 = ifelse(hhinc %in% 11:13, 1, 0), #S$11000-17500
                        Inc5 = ifelse(hhinc %in% 14:18, 1, 0), #S$17700-$20,000+
...
model_FINAL <- '
                      ...
                      ESAV_Likert_rev ~ Inc1 + Inc2 + Inc4 + Inc5 + Age + Male + University + FullEmploy + Kids +
                      license_holder + AV_Safe + OwnsCar + av_know +
                      ...
                      PSAV_Likert_rev ~ Inc1 + Inc2 + Inc4 + Inc5 + Age + Male + University + FullEmploy + Kids +
                      license_holder + AV_Safe + OwnsCar + av_know +
                      perfor_exp + effo_exp + soca_inf + fac_cond + hedo_motv + trust + tech_int + 
                      prh_trips_10 + srh_trips_2  + Nonmotor_superuser + Nonmotor_frequser + Nonmotor_reguser +
                      PT_superuser + PT_frequser + PT_reguser + 
